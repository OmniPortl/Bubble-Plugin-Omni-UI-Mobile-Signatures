function(props) {
    ...
    function buildHtml(initialConfig) {
        ...
        return "<!doctype html><html><head><meta name=\"viewport\" content=\"width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no\"><style>html,body{margin:0;padding:0;width:100%;height:100%;overflow:hidden;background:transparent;-webkit-user-select:none;user-select:none;touch-action:none;}#root{position:relative;width:100%;height:100%;overflow:hidden;}#pad{display:block;width:100%;height:100%;touch-action:none;}#placeholder{position:absolute;left:0;right:0;top:50%;transform:translateY(-50%);text-align:center;font:14px -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;opacity:.55;pointer-events:none;}</style></head><body><div id=\"root\"><canvas id=\"pad\"></canvas><div id=\"placeholder\"></div></div><script>(" + webSignatureRuntime.toString() + ")(" + configScript + ");</script></body></html>";
    }
    ...
    function webSignatureRuntime(initialConfig) {
        ...
        function resize() {
            ...
            displayWidth = Math.max(1, rect.width || window.innerWidth || 1);
            displayHeight = Math.max(1, rect.height || window.innerHeight || 1);
